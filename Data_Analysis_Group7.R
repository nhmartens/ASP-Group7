...
df_country_1 <- merge(df_returns[,c('Date', 'AEX_disc_return',
...
# insert Monday dummy
df_country_1$dMonday <- ifelse(weekdays(df_country_1$Date) == 'Monday', 1, 0)

# insert Before and After Vaccination dummy
df_country_1$dPeriod <- ifelse(df_country_1$Date <= dateranges_country_1[2],0,1)

# transform discrete to continuous returns
df_country_1$AEX_cont_return <- log(1+(df_country_1$AEX_disc_return/100))*100
df_country_1$Portfolio_cont_return <- 
  log(1+(df_country_1$Portfolio_disc_return/100))*100
df_country_1$MSCI_cont_return <- log(1+(df_country_1$MSCI_disc_return/100))*100
write.csv(df_country_1, paste0(dirdata,"df_country1.csv"), row.names = FALSE)
df_country1 <- df_country_1[complete.cases(df_country_1),]
write.csv(df_country1, paste0(dirdata,"df_country1.csv"), row.names = FALSE)

df_summary <- df_country1




stargazer(df_summary, order = c(1,2,10,3,9,5,6,4,11,7,8))
...
ggsave(paste0(dirprograms,'AEX_stationarity.png'),width = 15, 
       height = 10, units = "cm")
...
ggsave(paste0(dirprograms,'Real_Estate_stationarity.png'),width = 15, 
       height = 10, units = "cm")
...
Plot1 <- ggplot(df_country1, aes(x = new_cases_per_million, y = AEX_disc_return)) +
  geom_point(size = 0.7) + 
...
Plot2 <- ggplot(df_country1, aes(x = reproduction_rate, y = AEX_disc_return)) +
  geom_point(size = 0.7) + 
  stat_smooth(method = "lm", formula = y ~ x, size = 1) + 
...
Plot3 <- ggplot(df_country1, aes(x = new_cases_per_million, y = Portfolio_disc_return)) +
  geom_point(size = 0.7) + 
  stat_smooth(method = "lm", formula = y ~ x, size = 1) + 
  xlab("New Cases per Million") +
...
Plot4 <- ggplot(df_country1, aes(x = reproduction_rate, y = Portfolio_disc_return)) +
  geom_point(size = 0.7) + 
  stat_smooth(method = "lm", formula = y ~ x, size = 1) + 
  xlab("Reproduction Rate") +
  ylab("Real Estate Portfolio Returns")
...
ggsave(paste0(dirprograms,'Plot_AEX.png'),
       arrangeGrob(Plot1, Plot2,  ncol=2),
       width = 15, height = 7, units = "cm")

grid.arrange(Plot3, Plot4,  ncol=2)

ggsave(paste0(dirprograms,'Plot_RE.png'),
...
# Indicates serial correlation for mdl1, mdl3, mdl5, mdl6, mdl7 
# which are therfore reported with Newey-West SEs
...
stargazer(mdl1, mdl2, mdl3, mdl4, intercept.bottom = FALSE,
          se = list(SE_sc_1, SE_sc_2, SE_sc_3, SE_sc_4), no.space = TRUE,
          df = F)

stargazer(mdl5, mdl6, mdl7, mdl8, intercept.bottom = FALSE,
          se = list(SE_sc_5, SE_sc_6, SE_sc_7, SE_sc_8), no.space = TRUE,
          df = F)
...
# mdl1 has serially correlated errors
lmtest ::bptest(mdl2)
# mdl3 has serially correlated errors
lmtest ::bptest(mdl4)
# mdl5 has serially correlated errors
...
QQ1 <- qplot(sample = mdl1$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 1 - Discrete returns') + 
...
QQ2 <-qplot(sample = mdl2$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 2 - Discrete returns') + 
  theme(plot.title = element_text(hjust = 0.5, size = 11))
...
QQ3 <- qplot(sample = mdl3$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 3 - Discrete returns') + 
  theme(plot.title = element_text(hjust = 0.5, size = 11))
...
QQ4 <- qplot(sample = mdl4$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 4 - Discrete returns') + 
  theme(plot.title = element_text(hjust = 0.5, size = 11))
...
QQ5 <- qplot(sample = mdl5$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 1 - Continuous returns') + 
  theme(plot.title = element_text(hjust = 0.5, size = 11))
...
QQ6 <- qplot(sample = mdl6$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 2 - Continuous returns') + 
  theme(plot.title = element_text(hjust = 0.5, size = 11))
...
QQ7 <- qplot(sample = mdl7$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 3 - Continuous returns') + 
  theme(plot.title = element_text(hjust = 0.5, size = 11))
...
QQ8 <- qplot(sample = mdl8$residuals, stat='qq') + 
  geom_abline(slope = 1,intercept = 0) +
  xlab('Theoretical Quantiles') +
  ylab('Sample Quantiles') +
  ggtitle('Model 4 - Continuous returns') + 
  theme(plot.title = element_text(hjust = 0.5, size=11))

require(gridExtra)
grid.arrange(QQ1, QQ2, QQ3, QQ4,  ncol=4)

ggsave(paste0(dirprograms,'QQ1.png'),
       arrangeGrob(QQ1, QQ2, QQ3, QQ4,  ncol=2),
       width = 15, height = 13, units = "cm")

grid.arrange(QQ5, QQ6, QQ7, QQ8,  ncol=4)

ggsave(paste0(dirprograms,'QQ2.png'),
       arrangeGrob(QQ5, QQ6, QQ7, QQ8,  ncol=2),
       width = 15, height = 13, units = "cm")
...
stargazer(mdl1, mdl2, mdl3, mdl4, type='text', intercept.bottom = FALSE,
          se = list(SE_sc_1, SE_het_2, SE_sc_3, SE_het_4), df=F, 
...
stargazer(mdl5, mdl6, mdl7, mdl8, type='text', intercept.bottom = FALSE,
          se = list(SE_sc_5, SE_sc_6, SE_sc_7, SE_het_8), df=F, 
